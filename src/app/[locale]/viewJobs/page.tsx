...
const ViewJobs = () => {
  ...
  const [jobSeeker, setJobSeeker] = useState<any[]>([]);
  ...
  const handleApplyClick = async () => {
    try {
      ...
      const response = await fetch(
        `https://neo-814m.onrender.com/v1/post/apply/${jobId}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    <>
      <>
        <Box
          h={{ base: "auto", lg: "230px" }}
          bg="linear-gradient(to right, white , gray);"
        >
          <Box w={{ base: "100%", lg: "1200px" }} m="auto">
            <Flex
              justifyContent="space-between"
              flexDirection={{ base: "column", lg: "row" }}
              padding={{ base: "24px 80px", lg: "0" }}
            >
              <Box pt="40px">
                <Flex
                  gap="30px"
          ...
          <Flex flexDirection={{ base: "column", lg: "row" }}>
            ...
            >
              ...
              <Text fontWeight="bold">
                {t("Company.ViewJob.requiredSkills")}
              </Text>
              <Box p="30px 40px 0 0">
                <Text>
              ...
              <Flex
                ...
                flexDirection={{ base: "column", lg: "row" }}
              >
                <Text mb={{ base: "30px", lg: "0" }}>
                  {t.rich("Company.ViewJob.share", {
                    NavigationLink: (chunks) => (
                      <NavigationLink
                        style={{ color: "blue" }}
                        href={role === "COMPANY" ? "/postJobs" : "/register"}
                      >
                        {chunks}
                      </NavigationLink>
                    ),
            ...
            <Box>
              ...
              <LocationItem
                ...
                label="Maaş"
                content={`${
                  job?.maxEstimatedBudget ? job.maxEstimatedBudget : ""
                } ${job?.currency ? job?.currency : ""}`.trim()}
              />
              <LocationItem
