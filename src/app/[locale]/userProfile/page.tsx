...
import { Card, Box, CardBody, CardHeader, Heading, Flex, Button, Text, Input, useDisclosure, Modal, Link,ModalBody, ModalCloseButton, ModalContent, ModalFooter, ModalHeader, ModalOverlay, InputGroup, InputLeftAddon, InputRightAddon, FormControl, FormHelperText, FormLabel, Center, Divider, Textarea } from '@chakra-ui/react'
...
const Profile = () => {
    ...
    const handleSubmitExperience = async (e: React.ChangeEvent<HTMLInputElement>) => {
        e.preventDefault();
        const payload = {
        ...
        try {
            const response = await fetch(`https://neo-814m.onrender.com/v1/experience/${userProfileId}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    Token: `${token}`,
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch(`https://neo-814m.onrender.com/v1/education/jobSeeker/${userProfileId}`);
                if (!response.ok) {
            ...
            try {
                const response = await fetch(`https://neo-814m.onrender.com/v1/experience/jobSeeker/${userProfileId}`);
                if (!response.ok) {
                    throw new Error('Network response was not ok');
                }
                const data = await response.json();
            ...
            try {
                const response = await fetch(`https://neo-814m.onrender.com/v1/portfolio/jobSeeker/${userProfileId}`);
                if (!response.ok) {
                    throw new Error('Network response was not ok');
                }
                const data = await response.json();
                                            ...
                                            <Box pt="40px">
                                                <Flex gap="30px" alignItems="center" ml="20px">
                                                    <Box
                                                        ...
                                                        bg="gray"
                                                    >
                                                        <RiGraduationCapLine size="20px" color="white" />
                                                    </Box>
                                                    ...
                                                    <Flex flexDirection="column" >
                                                        ...
                                                        <Flex alignItems="center" gap="6px">
                                                            ...
                                                            <Text fontSize="14px" mr="20px">  {item.certificateDegreeName}
                                                            </Text>
                                                            <GoClock />
                                                            <Text fontSize="14px">
                                                                {new Date(item.startingDate).toLocaleDateString()} - {new Date(item.completionDate).toLocaleDateString()}
                                                            </Text>
                                                        </Flex>
                                                    </Flex>
                                                </Flex>
                                                ...
                                                <Flex gap="30px" alignItems="center" ml="20px">
                                                    ...
                                                    <Flex flexDirection="column"  gap="5px">
                                                        ...
                                                        <Flex alignItems="center" gap="6px">
                                                            ...
                                                            <GoClock />
                                                            <Text fontSize="14px">
                                                                {new Date(item.startingDate).toLocaleDateString()} - {new Date(item.endDate).toLocaleDateString()}
                                                            </Text>
                                                        </Flex>
                        ...
                        <ModalBody p={'3.7rem 7.4rem 0'}>
                            <Flex flexWrap={'wrap'} columnGap={'1.5rem'} width={'100%'}>
                                <FormControl minH={'7.1rem'} flexBasis={'calc(50% - 0.8rem)'}>
                                    ...
                                    <Input type='text' onChange={(e) => setCompanyName(e.target.value)} placeholder={t('Profile.Experience.name')} h={'3.1rem'}></Input>
                                </FormControl>
                                <FormControl minH={'7.1rem'} flexBasis={'calc(50% - 0.8rem)'}>
                                    <FormLabel fontSize={'1.1rem'} mb={'0.9rem'}>{t('Profile.Experience.title')}</FormLabel>
                                    <Input type='text' onChange={(e) => setTitle(e.target.value)} placeholder={t('Profile.Experience.title')} h={'3.1rem'}></Input>
                                </FormControl>
                                <FormControl minH={'7.1rem'} flexBasis={'calc(50% - 0.8rem)'}>
                                    <FormLabel fontSize={'1.1rem'} mb={'0.9rem'}>{t('Common.FormInputs.startingDate.label')}</FormLabel>
                                    <Input
                                            ...
                                            <Box pt="40px">
                                                <Flex gap="30px" alignItems="center" ml="20px">
                                                    ...
                                                    <Flex flexDirection="column" >
                                                        ...
                                                        <Flex alignItems="center" gap="6px">
                                                            <TiPin size="24px" />
                                                            <Link fontSize="14px" mr="20px" color="blue">  {item.websiteUrl}
                                                            </Link>
